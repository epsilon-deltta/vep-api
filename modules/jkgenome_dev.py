...
refFlat_path = '/%s/D/Sequences/hg19/refFlat_hg19.txt' % (homedir,)
...
def loadBlatOutput(blatOutputPath='%s/D/Sequences/gencode_24/wgEncodeGencodeCompV24lift37.txt.gz' % homedir,by='transID',blacklist=['NR_106988']):
...
def convertTrans2Genome(blatH,transID,transPos,transLen): # pos base-1
    ...
    for trans in blatH[transID]:
        ...
        if transLen != trans['exnLen']:
            # print
            # print transID, transLen, trans['exnLen']
            # raise Exception
            continue
...
class locus: # UCSC type
    ...
    def regionType(self):
        return getRegionType(loadBlatOutputByChr(),(self.chrom,self.chrSta+1,self.chrEnd))
