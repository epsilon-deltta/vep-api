...
parseParam = lambda l: dict([(k,v) for (k,v) in l])
...
def fn_clean(baseDir, outFilePostFix, deleteFilePostFix, htmlF):
...
def main(inputFilePathL, genSpecFn, sampN, projectN='test_yn', server='smc1', genome='hg19', sra_download=False, sra_dump=False, clean=False, transfer=False, delete_original=False):
    ...
    if sra_dump:
        ...
        htmlF.write('Extracting FASTQ from SRA: cd %s;fastq-dump %s.sra --split-files<br>' % (sraBase,inputFilePre))
        os.system('cd %s;fastq-dump %s.sra --split-files' % (sraBase,inputFilePre))
        ...
        inputFilePathL = ('%s/%s.1.%s.fq' % (sraBase,inputFilePre,init_file_ext),'%s/%s.2.%s.fq' % (sraBase,inputFilePre,init_file_ext))
    ...
    for i in range(len(specL)):
        ...
        if not logOK(baseDir,logFileN,specL[i]['logExistsFn']): #failed execution
        ...
        if specL[i]['clean']: # clean if user-specified
             fn_clean(baseDir, specL[i]['outFilePostFix'], specL[i]['deleteFilePostFix'], htmlF) # clean only if both log and results are integral
